...
var cropsBox = document.forms[0].crops;

/* verify acres text box entry is a positive number */
// function verify acres
function verifyAcres() {
    ...
    try {
        ...
        if (!(acresBox.value > 0)) {
            throw "Please enter a number of acres grater than 0.";
        }
    } catch (message) {
        validity = false;
        messageText = message;
...
function verifyMonths() {
    ...
    try {
        if (!(monthsBox.value >= 1 && monthsBox.value <= 12)) {
            throw "Please enter a number of acres grater than 12.";
        }
    } catch (message) {
        validity = false;
        messageText = message;
